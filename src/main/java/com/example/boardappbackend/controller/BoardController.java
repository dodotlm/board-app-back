package com.example.boardappbackend.controller;

import com.example.boardappbackend.dto.BoardDto;
import com.example.boardappbackend.dto.ResponseDto;
import com.example.boardappbackend.entity.CustomUserDetails;
import com.example.boardappbackend.service.BoardService;
...
public class BoardController {
    ...
    @PatchMapping
    public ResponseEntity<?> modify(@RequestPart("boardDto") BoardDto boardDto, @RequestPart(value = "uploadFiles", required = false) MultipartFile[] uploadFiles,
        ...
        try{
            ...
            if(uploadFiles != null && uploadFiles.length > 0) {
                Arrays.stream(uploadFiles).forEach(file -> log.info("uploadFiles: {}", file.getOriginalFilename()));
            }
            if(changeFiles != null && changeFiles.length > 0) {
                Arrays.stream(changeFiles).forEach(file -> log.info("changeFiles: {}", file.getOriginalFilename()));
            }
            log.info("modify boardDto: {}", originFiles);
            ...
            log.error("originFiles: {}", e.getMessage());
            responseDto.setStatusCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
            responseDto.setStatusMessage(e.getMessage());
            return ResponseEntity.internalServerError().body(responseDto);
        }
